...
\caption{Bảng thuật ngữ sử dụng trong báo cáo.}\label{tab:glossary}\\
\toprule
...
\midrule
\endhead
\code{Agent} & Tác tử xử lý một phần thông tin, trích xuất belief và ghi vào bộ nhớ dùng chung. \\
\code{SharedMemory} & Bộ nhớ dùng chung giữa các tác tử, chịu trách nhiệm lưu trữ belief, phát hiện xung đột và cập nhật trạng thái sau phân xử. \\
\code{BeliefVersion} & Một phiên bản thông tin được tác tử ghi vào bộ nhớ, gồm key, value, source, confidence, \(\theta\), evidence và conflict type. \\
\code{MemorySlot} & Đơn vị lưu trữ theo từng key trong bộ nhớ, gồm working value, danh sách phiên bản belief và trạng thái phân xử. \\
\code{Conflict} & Tình huống hai belief có cùng key nhưng khác value, cần được phân loại và giải quyết trước khi đưa vào working memory. \\
\code{ConflictClassifier} & Thành phần phân loại xung đột dựa trên cặp belief \((existing, incoming)\), thay vì dựa trên từng belief đơn lẻ. \\
\code{AdaptivePolicy} & Bộ định tuyến chính sách, lựa chọn policy phù hợp dựa trên loại xung đột C1--C4. \\
\code{Mechanism Layer} & Lớp cơ chế lõi, chứa schema belief, shared memory, classifier và các policy phân xử; không phụ thuộc trực tiếp vào benchmark. \\
\code{Adapter Layer} & Lớp thích nghi dữ liệu, chịu trách nhiệm ánh xạ benchmark cụ thể thành định dạng và tín hiệu mà Mechanism Layer sử dụng. \\
\(\theta\) & Thứ tự thời gian logic của belief, dùng để xác định quan hệ trước--sau giữa các thông tin. \\
\code{C1} & Temporal obsolescence: thông tin cũ từng đúng nhưng đã bị thay thế bởi thông tin mới hơn. \\
\code{C2} & Source disagreement: hai nguồn gần cùng thời điểm đưa ra hai giá trị khác nhau cho cùng một key. \\
\code{C3} & Scope mismatch: hai thông tin tưởng như mâu thuẫn nhưng thực ra đúng trong hai phạm vi hoặc ngữ cảnh khác nhau. \\
\code{C4} & Entity ambiguity: cùng một key hoặc tên bề mặt nhưng thực chất trỏ tới hai thực thể khác nhau. \\
\code{CR-Acc} & Conflict Resolution Accuracy, chỉ số phản ánh độ chính xác của câu trả lời sau khi hệ thống xử lý xung đột. \\
\code{EM} & Exact Match, chỉ số đánh giá câu trả lời bằng cách so khớp chính xác với đáp án chuẩn. \\
\code{Token cost} & Số token trung bình được sử dụng khi đưa ngữ cảnh vào mô hình để trả lời truy vấn. \\
\bottomrule
\end{longtable}
...
\subsection{Hệ tác tử LLM và nhu cầu bộ nhớ dài hạn}

Các nghiên cứu về tác tử LLM cho thấy mô hình ngôn ngữ lớn có thể được tổ chức thành các tác tử có khả năng quan sát, lập luận, hành động và phản hồi với môi trường. ReAct đề xuất cách kết hợp lập luận và hành động trong cùng một chuỗi xử lý, giúp mô hình không chỉ sinh câu trả lời mà còn biết lựa chọn hành động trung gian khi giải quyết nhiệm vụ \cite{yao2023react}. Generative Agents mở rộng hướng tiếp cận này bằng cách mô phỏng các tác tử có hành vi xã hội, trong đó bộ nhớ, phản xạ và lập kế hoạch đóng vai trò quan trọng để duy trì tính nhất quán của hành vi qua thời gian \cite{park2023generative}.

Tuy nhiên, khi số lượng tác tử tăng lên hoặc hội thoại kéo dài, vấn đề không chỉ là lưu thêm thông tin mà còn là kiểm soát chất lượng của thông tin được lưu. Nếu các tác tử cùng cập nhật một bộ nhớ chung mà không có cơ chế phân xử, hệ thống dễ gặp các lỗi như ghi đè thông tin đúng, giữ lại thông tin đã lỗi thời hoặc gộp nhầm các thực thể khác nhau. Vì vậy, dự án này không tập trung vào việc xây dựng thêm một agent end-to-end, mà tập trung vào lớp điều phối bộ nhớ nằm giữa quá trình trích xuất belief và quá trình trả lời truy vấn.

\subsection{Retrieval-augmented generation và bộ nhớ ngoài}

Retrieval-augmented generation sử dụng cơ chế truy hồi thông tin để đưa tài liệu liên quan vào ngữ cảnh mô hình, qua đó cải thiện khả năng trả lời các câu hỏi cần tri thức bên ngoài \cite{lewis2020rag}. Các hướng như MemGPT tiếp tục phát triển ý tưởng bộ nhớ ngoài, xem LLM như một hệ điều hành có thể quản lý nhiều vùng nhớ khác nhau nhằm vượt qua giới hạn context window \cite{packer2023memgpt}. Những nghiên cứu này cho thấy bộ nhớ ngoài là một thành phần quan trọng trong các hệ thống LLM dài hạn.

Điểm khác biệt của báo cáo này là không chỉ quan tâm đến việc truy hồi hoặc mở rộng bộ nhớ, mà tập trung vào câu hỏi: khi nhiều phiên bản thông tin mâu thuẫn cùng tồn tại trong bộ nhớ, hệ thống nên chọn, giữ, tách hay cách ly thông tin như thế nào. Nói cách khác, dự án đặt trọng tâm vào \textit{memory arbitration} thay vì chỉ \textit{memory retrieval}.

\subsection{Belief revision và truth maintenance}

Các lý thuyết belief revision nghiên cứu cách một hệ tri thức cập nhật niềm tin khi xuất hiện thông tin mới có thể mâu thuẫn với tri thức hiện có. Mô hình AGM đặt nền tảng cho việc sửa đổi, co rút và mở rộng tập niềm tin một cách có nguyên tắc \cite{agm1985}. Truth Maintenance System cũng đề xuất cách duy trì tính nhất quán của một hệ tri thức thông qua việc ghi nhận phụ thuộc và cập nhật trạng thái của các mệnh đề \cite{doyle1979}.

Dự án kế thừa tinh thần của các hướng này ở mức hệ thống: thông tin mới không được ghi đè tùy tiện, mà cần được đặt trong quan hệ với thông tin cũ, nguồn, bằng chứng và thứ tự thời gian logic. Tuy nhiên, thay vì xây dựng một hệ logic hình thức đầy đủ, báo cáo chuyển hóa ý tưởng đó thành một cơ chế thực dụng hơn cho hệ đa tác tử LLM: biểu diễn belief bằng schema, phân loại xung đột theo C1--C4 và chọn chính sách phân xử bằng AdaptivePolicy.

\subsection{MemoryAgentBench và đánh giá bộ nhớ trong tác tử}

MemoryAgentBench là benchmark đánh giá năng lực bộ nhớ của LLM agents thông qua các tương tác nhiều lượt và các bài toán cần cập nhật thông tin theo thời gian \cite{memoryagentbench,memoryagentbench_dataset}. Nhánh FactConsolidation trong CR split đặc biệt phù hợp để kiểm tra các tình huống thông tin cũ bị thay thế bởi thông tin mới, tức nhóm xung đột C1 trong taxonomy của dự án.

Tuy nhiên, benchmark này không bao phủ đầy đủ mọi dạng xung đột mà hệ thống đề xuất. Các trường hợp bất đồng nguồn, lệch phạm vi và nhập nhằng thực thể tương ứng với C2, C3 và C4 cần được kiểm tra thêm bằng dữ liệu mở rộng hoặc injected subset có kiểm soát. Vì vậy, báo cáo sử dụng MemoryAgentBench như một công cụ đánh giá ban đầu cho cơ chế xử lý thông tin lỗi thời, nhưng không xem benchmark này là nguồn sinh ra toàn bộ thiết kế của hệ thống.
